...
windowed AS (
    ...
    LEFT JOIN ods_event_log ev
          ...
          -- 主指标事件名由运行时变量给出（build_warehouse(metric_event=...) → SET VARIABLE）。
-- 之前这里写死 'interaction'：接真实数据（事件叫 rating）时 exposure 计数正常、
-- SRM 正常，**指标却全 0** —— 差异审计第一次跑就抓到了这个。
AND ev.event_name = '{metric_event}'
