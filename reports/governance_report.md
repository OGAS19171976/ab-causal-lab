...
### 1.6 读接口要凭据，凭据有生命周期，请求有限速
  在此之前这三件都没有（README 已知边界里写着）。三条都用真实接口/注册表
  量出来，**不靠 sleep**：过期与宽限期用显式的 `at=` 摆出来。

  ① **读接口不再匿名**（公开面只剩 `/healthz`、`/docs`、静态页面）：
    无凭据 GET /api/experiments  -> HTTP 401
    错 token GET                 -> HTTP 401
    viewer 凭据 GET              -> HTTP 200（读只要 viewer）
    无凭据 GET /healthz          -> HTTP 200（健康检查必须公开，否则监控先死）

  ② **有效期**（默认 90 天；0 = 永不过期。老库迁移后是『永不过期』，
     否则一次迁移就把所有在用的凭据踢下线）：
    有效期 1 天、两天前签发 -> expired（说过期，不说不认识）
    有效期 1 天、刚签发     -> ok

  ③ **轮换的宽限期**（换发是滚动动作，不是停机动作）：
    宽限 30 分钟：旧 ok / 新 ok（窗口内两个都有效）
    宽限 0        ：旧 expired（立即失效）
    宽限已过      ：旧 expired

  ④ **限速**：按身份分成读/写两个固定窗口（默认 600 / 120 次每分钟，
     刻意宽松 —— 它挡的是跑飞的脚本与猜 token，不是节流正常使用）：
    额度调成 3/分钟，连读 5 次 -> [200, 200, 200, 429]；429 上的 `Retry-After` 头：有
    诚实边界：固定窗口在窗口边界上允许**两倍突发**，而且它在**进程内存**里 ——
    多副本部署会退化成「每副本各自限速」，那时才需要把窗口挪到共享存储。
...
### 7.11 「没做」的清单：从「靠人偶然发现」到「检查集里直接红」
  ...
  **补的第二层**：第一层只管『登记过的』句子，而第四次栽的跟头恰恰是
  **没登记的那些正文句子**（路线图里 Sun-Abraham/BJS 早已实现在 did.py、
  比值数仓的施工口径标题还写着『下一次开工照着做』，而它上一段就写着
  『已经接通了』）。所以现在多一条覆盖面机检：正文里每句被标记的『没做』
  都要逐字挂号，挂了号的句子也必须在 README 里还在 —— 它查的是
  **有没有人管**，不是『做没做』（那做不到）。下面两行就是它自己的读数。

  [OK  ] 正文『没做』的覆盖面：被标记的句子 9 条，登记表 8 条（两头顶得上）

  机检 0 条『没做』：0 条仍成立，0 条已经过时。
  另有 8 条**无法机检**、只能在正文里逐字登记 ——「有没有人管」是机检的（两头顶不上就红），「做没做」还是只能人读。
  以及 2 条**取舍声明**（有意不做，不是欠账）。
  退出码：0（0 = 清单与事实一致）
...
### 8. 结论
  ...
  * 仍未做的（写在这里而不是留着让人误会）：
    ...
    - ~~静态 token 无过期、无轮换、无限速；读接口仍然匿名~~ **已补**（见第 1.6 节）：
      有效期（默认 90 天；老库迁移后为「永不过期」）、带宽限期的轮换、
      按身份分读/写两个窗口的限速（429 + `Retry-After`），读接口也要凭据。
      **边界**：仍然不是 OAuth —— 没有刷新令牌、撤销列表下发与设备绑定，
      所以 token 在有效期内泄露仍然等于该用户被冒充，`disable` 才是立即止损；
