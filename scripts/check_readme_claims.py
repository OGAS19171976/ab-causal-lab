...
CLAIMS: list[tuple[str, str, str]] = [
    ...
    # 序贯那一段的两个读数：修掉列序错位之后才有数（上一版真实侧是"0 天"）。
    # 数据是确定的（真实库由 MovieLens parquet 重建），所以这两个数可以进清单。
    ("差异审计：真实侧序贯轨迹的 max|z|（修好之后）",
     "real_vs_synthetic.md", "1.567"),
    ("差异审计：序贯末桶与主效应 z 的自洽检查",
     "real_vs_synthetic.md", "自洽检查"),
    ...
    # 断点回归（m3 2.9 节）：数值随 n / 次数变，所以只加文本类声明。
    ("M3·断点回归：带宽是偏差-方差置换",
     "m3_validation.md", "带宽就是偏差-方差置换"),
    ("M3·断点回归：MSE 最优带宽是给点估计的",
     "m3_validation.md", "MSE 最优带宽是给点估计的"),
    ("M3·断点回归：偏差校正 + 稳健方差把欠账补完",
     "m3_validation.md", "上一轮记下的欠账在这一轮补完了"),
    ("M3·断点回归：只减偏差不加方差那一档", "m3_validation.md", "0.8200"),
    ("M3·断点回归：稳健方差那一档回到名义", "m3_validation.md", "0.9500"),
    ("M3·断点回归：偏差带宽 b 的置换被量出来",
     "m3_validation.md", "偏差带宽 b 的置换"),
    ("M3·断点回归：操纵检验两端都量",
     "m3_validation.md", "操纵检验两端都量"),
]
