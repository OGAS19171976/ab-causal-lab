#!/usr/bin/env python
"""M4 的 CATE 区间：算得出来，但**校准不了** —— 主因是偏差，方差也另有一处。
...
而且这一轮把原因也定位了：主要是**点估计本身有偏**（方差当时也顺手修过一版，
但真正的跨树协方差是后来才补的，见 8c 节）
（τ̂ 与真 CATE 的相关只有 ~0.6）。任何以它为心的区间都盖不住真值。
这一点很重要 —— 它把"水平不可用"从一句经验之谈变成了一个有数字的解释。

两条路线
--------
1. **叶内方差 + 跨树影响函数合成**（`CausalForest.predict_with_se`）：
   每棵树的叶子给出 τ̂ 及其影响函数原料（honest 的 estimation 半样本），
   跨树把**影响函数相加** —— 跨树协方差因此自动进入（8c 节实测：它把 SE
   从真实抽样波动的 0.354 倍抬到 0.702 倍）。旧口径"跨树独立合成"作为
   反例保留在 `combine="independent"` 下，两边都报。
...
def main() -> int:
    ...
    for ml in (10, 20, 40, 80):
        ...
        emit(f"  {ml:>9}{r['analytic_len']:>12.4f}{r['boot_len']:>16.4f}"
             f"{r['analytic_cov']:>12.4f}{r['finite_share']:>13.3f}")
    emit("")
    emit("  **这条验收标准要撤回，而不是记「部分满足」** —— 它当初量的是错的东西。")
    emit("  · 旧 SE（跨树独立合成）确实随叶子变大单调收缩：它测的其实就是")
    emit("    **叶内**噪声（约 1/√n_叶子），叶子越大它越小，几乎是同义反复；")
    emit("  · 改成长短的影响函数相加之后，方差由整份样本的有效样本量决定。")
    emit("    另一组配置（n=1200、30 棵、depth=4、seed=3）实测**中位** SE：")
    emit("    min_leaf=10/20/40/80/160 → 0.1679 / 0.1675 / 0.1836 / 0.1684 / 0.1583")
    emit("    （10→160 只降 5.7%），而同一组数据上旧口径是 0.0980 → 0.0830 → 0.0601")
    emit("    （160 比 10 低 38.7%）。**叶子大小换的主要是偏差，不是方差。**")
    emit("  · 叶子变大真正买到的，是**能不能给出有限 SE**：有限点占比")
    emit("    0.477 → 1.000（见上表最后一列）—— 这一列才是这条标准该量的东西。")
    emit("  无论如何，**覆盖率不随它改善**（0.13~0.16 一路持平）：换叶子尺度")
    emit("  既没修好偏差也没修好方差，README 那条「改点估计」的结论仍然成立。")
    ...
    emit("  * 这一轮的贡献是把原因**量化**了。当时的判断是「不是方差算错，")
    emit("    而是**点估计有偏**」—— 前半句后来被推翻了一半（跨树协方差确实也算错了，")
    emit("    见 8c 节），后半句仍然成立且是主因：")
    ...
    emit("  * 当时还留了一句「仍未做：GRF 的完整渐近方差（含跨树协方差项）」。")
    emit("    **这一条后来补上了，见 8c 节** —— 结论是方差确实也算错了，")
    emit("    补上之后 SE 从真实波动的 0.35 倍抬到 0.70 倍，但覆盖率仍然上不去，")
    emit("    因为偏差是 SE 的 7 倍。")
    ...
    emit(f"      只有 {mean_a:.4f} / {mean_b:.4f} —— 这不是「前面的实现写错了」，"
         "而是**对象换了**；")
    ...
    # ---- 8c. 森林的单元级 SE：跨树协方差补上了 ----------------------------- #
    emit("\n### 8c. 森林的单元级 SE：跨树协方差（第四处同族错误）")
    emit("  第 4 节把「盖不住」归因于点估计有偏。这句话只对了一半：")
    emit("  **方差也确实算错了** —— `predict_with_se` 原来把各棵树的叶子方差按")
    emit("  **独立**合成（`sqrt(Σ σ²_b)/B`），而各棵树用的是同一批数据。")
    emit("  这与第 16 节（CS / SA 的聚合）、事件研究里的 lead 是**同一个错误**，")
    emit("  森林是第四处 —— 也是最后一处还活着的。")
    emit("")
    emit("  修法：把各棵树的**影响函数相加**再取方差：")
    emit("    ψ̄ = (1/B)Σ_b ψ_b，ψ_i = ±(Y_i − 该臂均值)/n_臂（按叶子的")
    emit("    estimation 半样本数），Var(τ̂(x)) = Σ_i ψ̄_i² —— 跨树协方差自动进来。")
    emit("    这正是 GRF 那套森林权重的写法（α_i(x)=(1/B)Σ_b α_{b,i}(x)）。")
    emit("  说清没做到的：这是**给定树结构**的方差（把权重当固定），")
    emit("  所以它不覆盖分裂/子采样本身带来的波动 —— 实测就是这个残差。")
    emit("")
    from ablab.validation.hte_audit import run_forest_se_audit

    fsa = run_forest_se_audit(
        n_replications=8 if args.quick else 30,
        bootstrap_reps=2 if args.quick else 6,
        bootstrap_draws=6 if args.quick else 15,
    )
    for line in fsa.summary().splitlines():
        emit("  " + line)
    emit("")
    emit("  深度敏感性（max_depth → 能拿到**有限** SE 的网格点占比）：")
    emit("    " + "　".join(f"depth={d}: {s:.3f}" for d, s in fsa.depth_finite))
    emit("  读法（三条，都必须带着走）：")
    emit("    1. **修对了**：逐点 新/旧 比值恒 >1（%.2f~%.2f，中位 %.2f），"
         % (fsa.ratio_min, fsa.ratio_max, fsa.ratio_median))
    emit("       不是碰巧某几个点；SE 与真实波动的逐点相关也从 %.3f 抬到 %.3f。"
         % (fsa.corr_independent, fsa.corr_influence))
    emit(f"    2. **仍偏小**：真实波动的 {fsa.influence_vs_truth:.3f} 倍而不是 1.0 上下 ——")
    emit("       因为这是「给定树结构」的方差，分裂与子采样本身的波动不在里面。")
    emit("       GRF 自己的公式也是这个性质，所以这一档就到这里为止。")
    emit(f"    3. **覆盖率没救回来，而且原因更清楚了**：可给有限区间的点只有"
         f" {fsa.finite_share * 100:.1f}%，")
    emit("       其余点拿到的是**无穷区间**（某棵树切出了退化叶子）—— 那是「没材料」，")
    emit("       不是「没问题」；而在有限的那部分点上，偏差是 SE 的"
         f" {fsa.finite_bias_over_se:.2f} 倍，")
    emit(f"       所以覆盖率反而更低（{fsa.coverage_influence_finite:.4f}；若把无穷区间也算成"
         f"「盖住」会得到 {fsa.coverage_influence_with_inf:.4f} ——")
    emit("       那个数好看，但它什么都没证明，所以不采用）。")
    emit("       **要让单元级区间可用，下一步只能改点估计（局部估计方程 / 更强平滑），")
    emit("       而不是继续修方差 —— 这与第 5 节的结论是同一条。**")
