...
def main() -> int:
    ...
    emit("")
    emit("  **补的第二层**：第一层只管『登记过的』句子，而第四次栽的跟头恰恰是")
    emit("  **没登记的那些正文句子**（路线图里 Sun-Abraham/BJS 早已实现在 did.py、")
    emit("  比值数仓的施工口径标题还写着『下一次开工照着做』，而它上一段就写着")
    emit("  『已经接通了』）。所以现在多一条覆盖面机检：正文里每句被标记的『没做』")
    emit("  都要逐字挂号，挂了号的句子也必须在 README 里还在 —— 它查的是")
    emit("  **有没有人管**，不是『做没做』（那做不到）。下面两行就是它自己的读数。")
    emit("")
    import subprocess as _sp
    ...
    emit("  `src/ablab/platform/static/index.html` 是**无构建步骤**的单文件页面")
    emit("  （一个 `api(path, opts)` helper 包住 fetch）。它最容易出的两类事故")
    emit("  都**不报错**：后端改了路径、前端静默 404；改了某个 id、控件静默失效。")
    emit("  这一轮给它配了四条机检（`scripts/check_frontend.py`，秒级、进快速组）：")
    emit("    1. **路由契约**：页面调用的每条端点都必须在服务端路由表里（含方法）；")
    emit("    2. **反向契约**：路由表里没出现在页面上的端点，必须有**写下来的决定**；")
    emit("    3. **选择器契约**：`$(「#x」)` / `querySelector(「.y」)` 指向的 id/class 必须存在；")
    emit("    4. **语法**：`node --check` 对页面里的 JS 做一次真正的解析。")
    emit("  路由表走**两条独立路径**取（静态解析装饰器 + 真的构造一次 app 读 routes），")
    emit("  两边算出来不一样就报错 —— 以后有人改用 include_router，静态那一条会漏。")
    emit("")
    for _line in _run_checker("check_frontend.py"):
        emit("    " + _line)
    emit("")
    emit("  第一次跑就抓出三件事，都不是页面写错了，而是**没人写下来的事实**：")
    emit("    · FastAPI **自带** 4 条路由（/docs、/redoc、/openapi.json 与 oauth2-redirect）")
    emit("      —— 第一版把它们报成「界面没用到且没有决定」4 条假阳性，现在单独归类；")
    emit("    · 路由表里有 12 条**界面有意不做**的端点，这一轮逐条写下理由")
    emit("      （删除/停实验/改判定口径这类不可逆或改变解释的动作，页面刻意不给入口）；")
    emit("    · 我自己的决定表里有一条其实**已经被 UI 用着**（`POST /api/validate/aa`），")
    emit("      被反向判据当场抓出来 —— 决定表与「没做」清单一样会漂，所以也要有反向检查。")
    emit("  诚实的边界：这四条检查的是**契约**（路径/方法/选择器/语法），不是**行为**。")
    emit("  「点了按钮会不会真的做对」靠 HTTP 层测试（`tests/test_platform_*.py`：状态码、")
    emit("  乐观锁 412、审计留痕）与人工看一眼；Playwright 那类端到端与它们重叠度高，")
    emit("  **刻意不做** —— 这一条也写在脚本的模块文档里，免得后来人以为漏了。")
    emit("  契约检查自己也有故障注入测试：拿一份**故意写坏**的页面（调用不存在的端点、")
    emit("  方法写错、选择器指向不存在、语法错）跑一遍，断言它**确实报红**。")

    emit("\n### 7.14 真实数据：把「没有真实流量」变成一道有证据的门")
    emit("  已知边界里原先写着「数据里到底有没有真实流量，只能人看」——")
    emit("  这一轮把它变成 `unimplemented.py` 里一条**机检项**：")
    emit("  `kind=file_absent`、`target=data/real/provenance.json`。")
    emit("  也就是说：只要那份 provenance 不存在，README 那句话就成立；")
    emit("  一旦有人把外部数据接进来，检查集立刻红，并逼着 README 改口径。")
    emit("  这是把一个**无法核对**的问题换成可核对的证据（与 7.11 节同一个机制）。")
    emit("")
    emit("  门本身还查三层（`scripts/check_real_traffic.py`）：")
    emit("    1. **契约**：source / exported_at / external_generator / experiments /")
    emit("       tables 都在，实验的设计权重和为 1（口径必须来自声明，不能从数据反推）；")
    emit("    2. **字节与声明一致**：每张表的 sha256 与行数都要和磁盘上的文件对得上；")
    emit("    3. **反冒充**：`external_generator` 必须为 true，且源目录里不许出现")
    ...
    for _line in _run_checker("check_real_traffic.py"):
        emit("    " + _line)
    emit("")
    emit("  契约被实测修正过一次，值得记：第一版把 `user_profile.reg_ds` 写成「可选」，")
    emit("  依据是 `_normalize_profile` 里那个 `if reg_ds in out.columns` 的写法。")
    emit("  端到端测试（拿最小 fixture 真的跑一遍链路）当场给出")
    emit("  `KeyError: [\'reg_ds\'] not in index` —— **归一化器认为可选，链路认为必需**。")
    emit("  所以现在门会先查必需列，缺列时报清楚哪张表缺哪列。")
    emit("  教训（写进 README 决策 55）：**契约要从链路写，不是从某一段代码写**。")
